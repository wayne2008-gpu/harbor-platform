...
第一阶段目标不是完整合成数据平台，而是先让 Harbor job 能被 API 提交、被 runner 消费、被 MySQL 查询状态，并能在本地 Compose 环境中用两个 runner 跑通。

## 排期假设

- 计划起点：2026-08-14。
- 人力假设：1 到 2 名平台后端研发，必要时 1 名同学协助云资源和测试数据集验证。
- 优先级：先打通 runner 边界，再接 MySQL/RocketMQ，最后做合成数据平台。
...
- `harbor-api` 将 job 写入 MySQL 并发布 RocketMQ 消息。
- 两个 `harbor-runner` 实例属于同一 consumer group，只有一个 runner 成功拿到 job lease。
...
- Docker Compose 能一键启动 MySQL、RocketMQ、harbor-api、两个 runner。
...
| M4 | 1.5 到 2 周 | MySQL/RocketMQ 集成 | lease、heartbeat、consumer、状态写回 |
...
| M8 | 2 周以上 | 云部署加固 | TencentDB、TDMQ RocketMQ、TKE runner、COS |
...
- 确认 MySQL schema 初稿。
- 确认 RocketMQ topic、consumer group、message body。
- 确认取消和 lease 语义。

交付物：

- `packages/harbor-service-contracts/` 的最小 package skeleton。
- 状态枚举和消息 schema。
- 状态转换单测。
- JSON round-trip 单测。
...
- RocketMQ message 只包含 job_id 和最小 routing metadata。
...
创建 `services/harbor-control-plane/` 的最小 FastAPI 服务。
...
- 新增 RocketMQ producer port 和 in-memory fake。
- 实现 endpoints：
  - `POST /jobs`
  - `GET /jobs/{job_id}`
...
- `POST /jobs` 写入 DB 后再 publish RocketMQ。
...
## M4: Runner MySQL 和 RocketMQ 集成

时间盒：1.5 到 2 周。

目标：

runner 从 RocketMQ 收消息，通过 MySQL lease 抢占 job，运行 Harbor，并写回状态。

任务：

- runner 注册到 MySQL。
- runner heartbeat。
- RocketMQ consumer 接入 `harbor-runners` consumer group。
- 实现原子 lease：
...
- terminal 后 ack RocketMQ message。
...
- 创建 `services/synthetic-data-platform/` 最小服务。
...
- M3/M4 控制面和调度链路：FastAPI、SQL repository、MySQL schema、RocketMQ producer/consumer、heartbeat、lease、expired lease recovery、runner offline marking、snapshot 回写和 trial 明细同步。
- M5 本地 Compose 栈：MySQL、RocketMQ、harbor-api、两个 runner、synthetic-data-platform；runner 镜像内已包含 Docker CLI/Compose 插件，并提供 rootless Docker TLS、rootless Docker host-path、rootless Docker legacy 与 AGS override 示例。
...
- 过期 `leased` job 会被重新排队，`acquire_lease` 在处理 RocketMQ redelivery 时也会先回收过期 lease。
...
- `docker compose -f compose.dev.yml -f compose.rootless-docker.yml config` 在设置示例 `HARBOR_RUNNER_DOCKER_HOST` 后可正常渲染。
- `docker compose -f compose.dev.yml -f compose.ags.yml config` 在使用占位 AGS 环境变量后可正常渲染；`smoke/ags-otel-bench-smoke-job.json` 已通过 JSON 和 Harbor `JobConfig` 静态校验，`harbor/generated/otel-bench-ags-smoke/go-http-tracing` 已通过 `Task.is_valid_dir` 和 `DatasetConfig.get_task_configs` 静态校验。
- `docker compose -f compose.dev.yml build harbor-runner-1` 已验证 runner 镜像可构建，镜像内 `docker --version` 为 27.5.1，`docker compose version` 为 v2.33.0。
- `compose.rootless-docker-tls.yml` 已通过 `docker compose config` 渲染校验；`scripts/create-rootless-docker-tls-certs.sh` 已通过 `bash -n` 和实际证书生成验证。
- `smoke/docker-touch-file-smoke-job.json` 已通过 JSON 和 Harbor `JobConfig` 静态校验；`harbor/generated/harbor-platform-docker-smoke/touch-file` 已通过 `Task.is_valid_dir` 和 `DatasetConfig.get_task_configs` 静态校验。
...
- rootless Docker TLS + host-path override 已完成 M5 真实 Docker provider smoke：`9a5e4f5364ba462bad0c51663cd9dd8a` 最终 `succeeded`，runner `runner-1`，`/trials` 有 1 条 succeeded trial，`/artifacts` 包含 job result 和 artifact manifest。
...
- runner 在 RocketMQ 本轮无消息且启用 `poll_control_plane_jobs` 时会 fallback 到控制面 queued-job polling，再由 MySQL lease 保证不会重复执行；该行为补齐了 Compose 双 runner 分担验证所需的空闲抢占能力。
- `cd harbor && uv run pytest tests/unit/runner -q` 已通过 23 个 runner 单测；`uv run ruff check src/harbor/runner tests/unit/runner` 已通过。
- `cd services/harbor-control-plane && uv run pytest tests -q` 已通过 32 个控制面单测；`uv run ruff check src tests` 已通过。
- 本轮复跑 `cd services/harbor-control-plane && uv run pytest tests -q`、`cd harbor && uv run pytest tests/unit/runner -q`、`cd services/synthetic-data-platform && uv run pytest tests -q` 均通过，覆盖控制面、runner、synthetic 三段当前回归。
- `cd services/synthetic-data-platform && uv run pytest tests -q` 已通过 8 个 synthetic 平台单测；`uv run ruff check src tests` 已通过，其中覆盖 succeeded Harbor job 通过 harbor-api artifacts endpoint ingest samples 的成功路径。

仍未满足：

- AGS/TKE 真实执行仍需要明确云资源授权和凭证；M5 Docker provider 已用 rootless Docker TLS 完成真实 succeeded job 验收。
...
其中 `run-once` 是最高风险边界。它决定 Harbor 现有 job 结果、进程生命周期、取消语义能否被稳定封装。不要在 `run-once` 稳定前同时推进完整 API 和 RocketMQ 集成。
...
```bash
cd services/harbor-control-plane
uv run pytest -q
...
```bash
cd services/synthetic-data-platform
...
| RocketMQ redelivery | job 重复执行 | 所有执行前必须先拿 MySQL lease |
...
2. 若要扩大真实后端覆盖面，使用 `deploy/docker-compose/compose.ags.yml` 和 `deploy/docker-compose/smoke/ags-otel-bench-smoke-job.json`，在明确授权云资源调用后跑 AGS succeeded smoke。
3. 后续 M8 将 runner-local artifact proxy 切换到 COS/object storage，并把 MySQL/RocketMQ 替换为云上托管服务。
